...
import androidLogo from '@assets/image_1776363323781.png';
import win10Logo from '@assets/image_1776363340534.png';
import win11Logo from '@assets/hd-windows-11-logo-icon-transparent-background-701751694967888_1776363414368.png';
...
function DeviceLogo({ device, size = 72 }: { device: DeviceInfo; size?: number }) {
  const { os, osVersion, type } = device;
  if (os === 'Android') return <img src={androidLogo} alt="Android" style={{ width: size, height: size, objectFit: 'contain' }} />;
  if (os === 'Windows') {
    const is11 = parseInt(osVersion, 10) >= 11;
    return <img src={is11 ? win11Logo : win10Logo} alt={`Windows ${osVersion}`} style={{ width: size * 0.85, height: size * 0.85, objectFit: 'contain' }} />;
  }
  const iconSize = Math.round(size * 0.8);
  if (os === 'iOS' || os === 'iPadOS' || os === 'macOS') return <Apple size={iconSize} strokeWidth={1.3} fill="currentColor" />;
  if (os === 'ChromeOS') return <Chrome size={iconSize} strokeWidth={1.3} />;
  if (os === 'Linux') return <Terminal size={iconSize} strokeWidth={1.3} />;
...
export default function PhoneModelDetector() {
  ...
  // AI Privacy Score — computed whenever scan is complete
  ...
  // Compute timezone mismatch
  const tzMismatch = datetime && ipInfo?.timezone && datetime.browserTimezone !== ipInfo.timezone;

  // Format time in a given timezone using the live clock
  function formatInTz(tz: string): string {
    try {
      return liveTime.toLocaleString('en-GB', {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-x-hidden">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-10 sm:pt-14 pb-16">
        ...
        {/* AI PRIVACY SCORE — shown after all sections complete */}
        {privacyScore && (
        ...
        {/* SECTION GRID — 2-column layout like pixelscan.net */}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <SectionCard {...SECTION_META.device} status={statuses.device} onScan={scanDevice} locked={isLocked}>
                {device && <DeviceSectionBody device={device} />}
              </SectionCard>
            </div>
            ...
            <SectionCard {...SECTION_META.ip} status={statuses.ip} onScan={scanIp} locked={isLocked}>
              {ipInfo && (
                <div className="grid grid-cols-2 sm:grid-cols-2 gap-x-6 gap-y-5">
                  ...
                  <KvCell label="WebRTC IP" value={null} mono />
                  <KvCell label="Country" value={ipInfo.country ? `${ipInfo.country}${ipInfo.countryCode ? ' (' + ipInfo.countryCode + ')' : ''}` : null} />
                  <KvCell label="City" value={ipInfo.city} />
                  <KvCell label="ISP / Organization" value={ipInfo.isp} wide />
                  <KvCell label="ASN" value={ipInfo.asn} mono />
                  <KvCell label="Region / State" value={ipInfo.region} />
                  <KvCell label="Postal Code" value={ipInfo.postal} mono />
                  <KvCell label="Timezone" value={ipInfo.timezone} mono />
                  <KvCell label="Latitude" value={ipInfo.latitude !== null ? ipInfo.latitude.toFixed(4) : null} mono />
                  <KvCell label="Longitude" value={ipInfo.longitude !== null ? ipInfo.longitude.toFixed(4) : null} mono />
                  <KvCell label="User Type" value="isp" mono />
                </div>
              )}
              {statuses.ip === 'error' && <div className="text-[13px] text-destructive">Could not resolve IP information.</div>}
            </SectionCard>
              ...
              {browser && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                    <KvCell label="HTTP" value={`${browser.browser} ${browser.browserVersion}`.trim()} />
                    <KvCell label="JavaScript" value={browser.ua.substring(0, 60) + '...'} />
                    <KvCell label="Platform" value={browser.platform} mono />
                    <KvCell label="Do Not Track" value={browser.doNotTrack ?? 'null'} mono />
                    <KvCell label="Hardware Concurrency" value={browser.languages.length > 0 ? String(navigator.hardwareConcurrency || 0) : null} mono />
                  </div>
                  <div className="pt-3 border-t border-white/[0.05]">
                    <div className="text-[11px] font-medium text-muted-foreground/55 mb-2">Full User-Agent</div>
                    <div className="text-[11px] text-foreground/70 font-mono break-all leading-relaxed">{browser.ua}</div>
                  </div>
                </div>
              )}
            </SectionCard>
              ...
              {datetime && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <KvCell label="Time from JS" wide value={formatInTz(datetime.browserTimezone)} mono />
                    {ipInfo?.timezone
                      ? <KvCell label="Time from IP" wide value={formatInTz(ipInfo.timezone)} mono />
                      : <KvCell label="Time from IP" wide value="Run IP scan to compare" />
                    }
                  </div>
                  {tzMismatch && (
                    <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl" style={{ background: 'rgba(255,180,0,0.08)', border: '1px solid rgba(255,180,0,0.2)' }}>
                      <AlertTriangle size={14} className="text-yellow-400 flex-shrink-0 mt-0.5" />
                      <div>
                        <div className="text-[12px] font-bold text-yellow-400">Timezone Mismatch Detected</div>
                        <div className="text-[11px] text-yellow-400/75 mt-0.5">
                          Browser: <span className="font-mono">{datetime.browserTimezone}</span> · IP: <span className="font-mono">{ipInfo?.timezone}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  {!tzMismatch && ipInfo?.timezone && (
                    <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl" style={{ background: 'rgba(0,212,106,0.07)', border: '1px solid rgba(0,212,106,0.15)' }}>
                      <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                      <span className="text-[12px] font-semibold text-primary/90">Timezones match — no VPN/proxy detected</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <SectionCard {...SECTION_META.hardware} status={statuses.hardware} onScan={scanHardware} locked={isLocked}>
              {hardware && (
                <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                  <KvCell label="WebGL Vendor" value={hardware.gpu.split('(')[0].trim()} />
                  <KvCell label="WebGL Version" value="WebGL 1.0" />
                  <KvCell label="CPU Cores" value={hardware.cores} mono />
                  <KvCell label="Memory" value={hardware.ram} mono />
                  <KvCell label="Screen Resolution" value={hardware.screenRes} mono />
                  <KvCell label="Viewport Size" value={hardware.viewportRes} mono />
                  <KvCell label="Pixel Ratio" value={`${hardware.pixelRatio}×`} mono />
                  <KvCell label="Color Depth" value={`${hardware.colorDepth}-bit`} mono />
                  <KvCell label="Touch Points" value={hardware.touchPoints} mono />
                  <KvCell label="Device Class" value={hardware.type} />
                  <KvCell label="Canvas Fingerprint" value={hardware.canvasHash} mono wide />
                </div>
              )}
            </SectionCard>
            ...
            <SectionCard {...SECTION_META.network} status={statuses.network} onScan={scanNetwork} locked={isLocked}>
              {network && (
                <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                  <KvCell label="Connection Type" value={network.net.effectiveType?.toUpperCase()} mono />
                  <KvCell label="Downlink" value={network.net.downlink !== null ? `${network.net.downlink} Mbps` : null} mono />
                  <KvCell label="Round-Trip Time" value={network.net.rtt !== null ? `${network.net.rtt} ms` : null} mono />
                  <KvCell label="Data Saver" value={network.net.saveData === null ? null : network.net.saveData ? 'Enabled' : 'Disabled'} />
                  <KvCell label="WebRTC IPs Found" value={network.webrtcIps.length} mono />
                  <KvCell label="Local IP(s)" value={network.webrtcIps.length ? network.webrtcIps.join(', ') : 'Not exposed'} mono wide />
                </div>
              )}
            </SectionCard>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div className="text-[13px] text-muted-foreground/80">
                        <span className="text-primary font-bold text-xl">{fonts.length}</span>{' '}
                        <span className="text-sm">fonts detected</span>
                      </div>
                    </div>
                    {fonts.length === 0 ? (
                      <div className="text-[13px] text-muted-foreground/60 italic">No system fonts detected (your browser may be blocking this).</div>
                    ) : (
                      <div className="flex flex-wrap gap-2 max-h-56 overflow-y-auto pr-1">
                        {fonts.map((f) => (
                          <span key={f} className="px-2.5 py-1 rounded-md text-[11px] font-mono"
                            style={{ background: 'rgba(0,212,106,0.08)', color: '#7be3ad', border: '1px solid rgba(0,212,106,0.14)' }}>
                            {f}
                          </span>
                        ))}
                      </div>
        ...
        <section className="mb-16 max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-3">How it works</div>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight" style={{ letterSpacing: '-0.03em' }}>
              One click. Full profile.
            </h2>
          </div>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { step: '01', title: 'Click the button', body: 'Press "Check My Device" — no account, no install, no permissions required. Works on any browser.' },
              { step: '02', title: 'Browser APIs are queried', body: 'We read User-Agent, hardware APIs, canvas renderer, network info, IP data, WebRTC, and installed fonts.' },
              { step: '03', title: 'See your full profile', body: 'Results appear instantly across 7 sections. Your Trust Score is calculated from all signals combined.' },
            ].map(({ step, title, body }) => (
              <div
                key={step}
              ...
              >
                <div className="text-[11px] font-black mb-3 font-mono" style={{ color: 'rgba(0,212,106,0.6)' }}>{step}</div>
                <div className="text-[15px] font-bold text-white mb-2">{title}</div>
                <p className="text-[13px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.4)' }}>{body}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-16 max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-3">Coverage</div>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight" style={{ letterSpacing: '-0.03em' }}>
              What gets scanned
            </h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Device Model', sub: 'Phone, tablet, desktop' },
              { label: 'IP & Location', sub: 'City, ISP, ASN, coords' },
              { label: 'Browser Info', sub: 'UA, platform, languages' },
              { label: 'Timezone Check', sub: 'JS vs IP comparison' },
              { label: 'Hardware', sub: 'CPU, RAM, GPU, screen' },
              { label: 'Canvas Hash', sub: 'Unique GPU fingerprint' },
              { label: 'WebRTC Leaks', sub: 'Local IP exposure check' },
              { label: 'Font Profile', sub: '30+ system fonts tested' },
            ].map(({ label, sub }) => (
              <div
                key={label}
              ...
              >
                ...
                <div className="text-[11px]" style={{ color: 'rgba(255,255,255,0.35)' }}>{sub}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-3xl mx-auto mb-4">
          <div className="text-center mb-8">
            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-3">FAQ</div>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight" style={{ letterSpacing: '-0.03em' }}>Common questions</h2>
          </div>
...
function DeviceSectionBody({ device }: { device: DeviceInfo }) {
  const accent = getBrandAccent(device);
  return (
    <div className="grid sm:grid-cols-[auto_1fr] gap-6 items-center">
      <div className="flex items-center justify-center sm:justify-start">
        <div className="relative">
          <div className="absolute -inset-6 rounded-full pointer-events-none" style={{ background: `radial-gradient(circle, rgba(${accent.color},0.22) 0%, transparent 65%)` }} />
          <div className="relative flex items-center justify-center" style={{ width: 110, height: 110, color: `rgb(${accent.color})` }}>
            <DeviceLogo device={device} size={92} />
          </div>
        </div>
      </div>
      <div className="text-center sm:text-left min-w-0">
        <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-2">Identified</div>
        <div className="text-2xl sm:text-4xl text-foreground mb-2 break-words leading-[1.0]"
          style={{ fontFamily: "'Inter', sans-serif", fontWeight: 900, letterSpacing: '-0.04em' }}>
          {device.displayName}
        </div>
        <div className="text-xs text-muted-foreground font-mono">
          {device.modelCode !== '—' ? `Model code · ${device.modelCode}` : `${device.os} ${device.osVersion}`.trim()}
        </div>
        <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 gap-x-6 gap-y-3">
          <KvCell label="OS" value={`${device.os} ${device.osVersion}`.trim()} />
          <KvCell label="Browser" value={`${device.browser} ${device.browserVersion}`.trim()} />
          <KvCell label="Device Type" value={device.type} />
  ...
  return (
    <div className="glass-card rounded-xl overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-white/[0.02] transition-colors">
        <span className="font-semibold text-[14px] text-foreground/90">{q}</span>
        <span className="text-primary/50 ml-3 flex-shrink-0">{open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</span>
      </button>
      {open && <div className="px-5 pb-4 text-[13px] text-muted-foreground leading-relaxed">{a}</div>}
    </div>
  );
}
